	...
	<div>
		<div class="page-title mb-16">
			<el-breadcrumb separator="/">
				...
				<el-breadcrumb-item>项目关闭通知单审批</el-breadcrumb-item>
				<el-breadcrumb-item>待审批的项目关闭通知单</el-breadcrumb-item>
			</el-breadcrumb>
		</div>
				...
				<el-table :data="tableData.data" stripe border class="mb-16" v-loading='isLoading' element-loading-text='数据加载中...'>
					<el-table-column prop="" label="操作" width="60px" tooltip-effect='dark'> 
						<template slot-scope="scope">
							<el-button type="text" @click="handleAudit(scope.$index,scope.row)">审批</el-button>
						</template>
					</el-table-column>
...
<script>
export default {
	name: 'ProCloseListToAudit',
	data () {
		return {
			searchUrl: '/api/project-close/order/list?status=0',
			tableData:{
				data:[],
				totalNum:0,
			    pageNum:0,
	...
	methods:{
		...
		handleView(index,row){
            let curId = row.closeListId;
            this.$router.push({
            	name:'CloseListAudit', params:{ id: curId}
            })
		},
		// 翻页 表格当前页码改变触发事件
