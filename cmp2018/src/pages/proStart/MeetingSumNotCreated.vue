...
export default {
	...
	mounted (){
		// status 0-未创建
		this.axios.get('/api/conference/list?status=0').then((res)=>{
			let data = res.data;
			if(data.code == 200){
	...
	methods:{
		...
		handleCreate(index, row){
			const curId = row.managerId;
			const name = row.projectName;
            this.$router.push({
            	name:'MeetingSumCreate', params:{ id: curId, name: name}
            })
		},
		// 翻页 表格当前页码改变触发事件
		handleCurrentChange(val){
			this.axios.get('/api/conference/list?status=0&pageNum='+val).then((res)=>{
				let data = res.data;
				if(data.code == 200){
					let model = data.model;
					this.tableData = model;
