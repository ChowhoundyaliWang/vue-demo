	...
	<div>
		<div class="page-title mb-16">
			<el-breadcrumb separator="/">
				...
				<el-breadcrumb-item>项目任务书审核</el-breadcrumb-item>
...
export default {
	...
	mounted (){
		...
		this.axios.get('/api/task/list?status=5').then((res)=>{
			let data = res.data;
			if(data.code == 200){
	...
	methods:{
		...
		handleAudit(index,row){
            let curId = row.id;
            this.$router.push({
            	name:'TaskBookAuditing', params:{ id: curId}
            })
		},
		viewUpdate(id){
		...
		handleCurrentChange(val){
			this.axios.get('/api/task/list?pageNum='+val+'&status=5').then((res)=>{
				let data = res.data;
				if(data.code == 200){
					let model = data.model;
					this.tableData = model;
