<template>
	<div>
		...
		<div class="page-content search-page">
				...
				<el-table :data="tableData.data" stripe border class="mb-16" v-loading='isLoading' element-loading-text='数据加载中...'>
					<el-table-column prop="operate" label="操作" width="60px" tooltip-effect='dark'> 
						<template slot-scope="scope">
							<el-button type="text" @click="handleView(scope.row.projectBudgetId, scope.row.taskId)">查看</el-button>
						</template>
					</el-table-column>
					<el-table-column prop="projectName" label="项目名称" width='320px'  show-overflow-tooltip>
						<template slot-scope='scope'>
							<span v-if="scope.row.updated">【<el-button type="text" v-on:click="viewUpdate(scope.row.projectBudgetId)">更新记录</el-button>】</span>{{scope.row.projectName}}
						</template>
					</el-table-column>
		    ...
		    <budget-contrast></budget-contrast>
		</div>
	</div>
</template>
...
import budgetContrast from '../../components/budgetContrast.vue'
import { mapActions } from 'Vuex'
export default {
	name: 'ProBudgetSumbitted',
	components:{
		"search": search,
		'record-dialog': recordDialog,
		"budget-contrast": budgetContrast
	},
	data () {
		return {
			isLoading: true,
			...
			listInfos:{
				curType: 'projectBudget',
			}
		}
	},
	mounted (){
		this.axios.get('/api/projectBudget/list?status=0').then((res)=>{
			const data = res.data;
			if(data.code == 200){
	...
	methods:{
		...
		handleView(curId, taskId){
            this.$router.push({
            	name:'ProBudgetView', params:{ id: curId, taskId: taskId }
            })
		},
		//点击查看更新记录列表
		viewUpdate(curId){
			this.axios.get("/api/projectBudget/record/list?id="+ curId).then((res)=>{
				let data = res.data;
				if(data.code == 200){
		...
		handleCurrentChange(val){
			this.axios.get('api/projectBudget/list?status=0&pageNum='+val).then((res)=>{
				let data = res.data;
				if(data.code == 200){
					let model = data.model;
					this.tableData = model;
