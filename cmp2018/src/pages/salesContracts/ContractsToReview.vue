				...
				<el-table :data="tableData.data" stripe border class="mb-16" v-loading='isLoading' element-loading-text='数据加载中...'>
					<el-table-column prop="" label="操作" width="60px" tooltip-effect='dark'> 
						<template slot-scope="scope">
							<el-button type="text" @click="handleReview(scope.$index,scope.row)">评审</el-button>
						</template>
					</el-table-column>
...
export default {
	...
	data () {
		return {
			status: 4,
			searchUrl: '/api/contract-review/list?status=0',
			tableData:{
				data:[],
				totalNum:0,
			    pageNum:0,
	...
	methods:{
		...
		handleReview(index,row){
            ...
            this.$router.push({
            	name:'ContractReview', params:{ id: curId}
            })
		},
		//表格搜索过滤事件
		doFilter(){
