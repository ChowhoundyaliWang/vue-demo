	...
	<div>
		<div class="page-title mb-16">
			<el-breadcrumb separator="/">
				...
				<el-breadcrumb-item>项目计划书</el-breadcrumb-item>
				<el-breadcrumb-item>未通过的项目计划书</el-breadcrumb-item>
			</el-breadcrumb>
		</div>
...
export default {
	...
	mounted (){
		this.axios.get('/api/planPaper/list?status=4').then((res)=>{
			let data = res.data;
			if(data.code == 200){
	...
	methods:{
		...
		handleCurrentChange(val){
			this.axios.get('/api/planPaper/list?status=4&pageNum='+val).then((res)=>{
				let data = res.data;
				if(data.code == 200){
					let model = data.model;
